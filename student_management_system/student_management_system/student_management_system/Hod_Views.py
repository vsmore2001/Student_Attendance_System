...
def DELETE_COURSE(request,id):
    course = Course.objects.get(id = id)
    course.delete()
    messages.success(request,'Course Is Successsfully Deleted !')
    return redirect('view_course')
...
@login_required(login_url='/')
def STAFF_FEEDBACK_REPLY(request):                            # use as STAFF_FEEDBACK
...
@login_required(login_url='/')
def STAFF_FEEDBACK_REPLY_SAVE(request):                        # use as STAFF_FEEDBACK_SAVE
    if request.method == "POST":
